...
SELECT SUM ( sueldo + NVL(comision,0))as importe FROM RECURSOS.empleado where iddepartamento=103;

--Desarrolle una sentencia SELECT para encontrar el mayor y menor sueldo en el departamento de ventas. Esquema EDUCA.
SELECT min(sueldo) as menor_sueldo, MAX(sueldo) as mayor_sueldo FROM RECURSOS.empleado;
...
SELECT count(*) as cant_empleados FROM  RECURSOS.empleado where iddepartamento=103
...
SELECT cur_matriculados AS CANT_MATRICULADOS,   (cur_matriculados * cur_precio) AS IMPORTE FROM  EDUCA.CURSO WHERE cur_nombre='SQL Server Administración'
...
SELECT cur_id, cur_nombre,   (cur_matriculados * cur_precio) AS IMPORTE FROM  EDUCA.CURSO
...
SELECT iddepartamento , COUNT(*) as cantidad FROM RECURSOS.empleado group by iddepartamento 

--Se necesita saber cuántos empleados han ingreso por año en cada departamento. Esquemas SCOTT y HR.
SELECT iddepartamento , COUNT (*) cantidad, YEAR(fecingreso) anio FROM RECURSOS.empleado group by iddepartamento ,YEAR(fecingreso)
order by 1, 3
...
SELECT iddepartamento , COUNT (*) cantidad_empleados, SUM(sueldo) as importe , AVG(sueldo)  promedio FROM RECURSOS.empleado group by iddepartamento ,YEAR(fecingreso)
...
SELECT  E.IDDEPARTAMENTO, E.IDEMPLEADO,  MAX(SUELDO) FROM RECURSOS.EMPLEADO   E INNER  JOIN 
(SELECT IDDEPARTAMENTO,  MAX(SUELDO)  SALARIO  FROM RECURSOS.EMPLEADO
 group by IDDEPARTAMENTO  ORDER BY  IDDEPARTAMENTO ) B ON E.IDDEPARTAMENTO=B.IDDEPARTAMENTO  AND E.SUELDO=B.SALARIO
 GROUP BY  E.IDDEPARTAMENTO,E.IDEMPLEADO  ORDER BY  IDDEPARTAMENTO 
...
SELECT C.IDCARGO,C.NOMBRE,  COUNT(*) AS CANTIDAD  FROM RECURSOS.EMPLEADO E INNER JOIN  CARGO C ON E.IDCARGO=C.IDCARGO 
...
--Se necesita saber que empleados han registrado menos de 5 movimientos. Esquema EUREKA.
 SELECT CHR_EMPLCODIGO ,COUNT(*) AS CANTIDAD  FROM EUREKA.MOVIMIENTO GROUP BY  CHR_EMPLCODIGO HAVING COUNT(*)<5
